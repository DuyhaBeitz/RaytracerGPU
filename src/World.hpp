...
public:
    ...
    World() {
        mats[0] = Mat(GRAY, LAMBERTIAN, 0.0, 0.0);
        mats[1] = Mat(GREEN, LAMBERTIAN, 0.0, 0.0);
        mats[2] = Mat(WHITE, METAL, 0.0, 0.0);
        mats[3] = Mat(WHITE, DIELECTRIC, 0.0, 1.5);

        objects[0] = Hittable::Sphere(Vector3{0.0, -1000.0, 0.0}, 1000, 0);
        objects[1] = Hittable::Sphere(Vector3{0.0, 1.0, 0.0}, 1, 3);
    }
...
private:
    ...
    void ApplyMaterialUniforms(Shader shader) {
        ...
        for (int i = 0; i < MATERIAL_COUNT; i++) {
            u_albedo[i] = FromColor(mats[i].albedo);
